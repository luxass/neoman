...
export default function WorldGenerator(ctx: { name: string }): NeomanGenerator {
  return {
    ...
    writing: ({ copy, templatePath, destinationPath, installDependencies }) => {
      copy(
        ...
        destinationPath("package/package.json"),
        ctx
      );
      copy(
        templatePath("_package.json"),
        destinationPath("package/hello/package.json"),
        ctx
      );
      copy(templatePath("_package.json"), destinationPath("package.json"), ctx);
      copy(
        templatePath("thisisadirectory"),
        destinationPath("newdir/thisisadirectory")
      );
